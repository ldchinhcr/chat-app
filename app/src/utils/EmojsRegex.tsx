...
export const EmojiReg = (text: string) => {
  let matchArr
  let lastOffset = 0
  const regex = new RegExp(
    '[\u{1f300}-\u{1f5ff}\u{1f900}-\u{1f9ff}\u{1f600}-\u{1f64f}' +
      ...
      '\u{25b6}\u{23f8}-\u{23fa}]',
    'gu'
  )
  const getByNative = (native: string) => {
  ...
  while ((matchArr = regex.exec(text)) !== null) {
    ...
    if (previousText.length) {
      partsOfTheMessageText.push(previousText)
    }
    ...
    const emojiId = (emoji as EmojiData)?.id

    if (emojiId) {
      partsOfTheMessageText.push(
        <Emoji emoji={emojiId} size={22} set="facebook" />
      )
    } else {
  ...
  if (Boolean(finalPartOfTheText?.length)) {
