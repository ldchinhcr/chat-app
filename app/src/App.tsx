...
function App() {
  ...
  useEffect(() => {
    socket.on(SOCKET_EVENTS.allRoom, (rooms: ChatRoom[]) => {
      dispatch({ type: CHAT_ACTIONS.SET_ROOM, payload: rooms })
    })
    socket.on(SOCKET_EVENTS.selectedRoom, (room: ChatRoom) => {
      dispatch({ type: CHAT_ACTIONS.CURRENT_ROOM, payload: room })
    })
    socket.on(SOCKET_EVENTS.setUser, (user: User) => {
      setUser(user)
      dispatch({ type: CHAT_ACTIONS.SET_USER, payload: user })
    })
    socket.on(SOCKET_EVENTS.messages, (msgs: Message[]) => {
      dispatch({ type: CHAT_ACTIONS.SET_MESSAGES, payload: msgs })
    })
    socket.on(SOCKET_EVENTS.oldmessages, (msgs: Message[]) => {
